...
.toc-list a {
    ...
    white-space: nowrap;
    display: block;
    touch-action: manipulation;   /* 모바일 탭 지연·제스처 충돌 방지 */
}
.toc-list a:hover {
...
/* 2026-09-22 PI: 배너 위에는 '이번 호가 무엇인가'(호수 · 발간일)를, 배너 아래에는
   ...
   배너 위 = 「2026 한국교육학회 제주지회 뉴스레터」
   배너 아래 = 「Since 1993 · 제2호」 (PI 2026-09-22 최종).
   연도와 호수를 떼어 놓았다 — 「2026 · 제2호」로 붙여 두면 '2026년의 두 번째 호'로
   읽히기 때문이다. 호수는 연혁(Since 1993) 옆에 두어 쌓인 횟수로 읽히게 한다.
   ★ 다만 1990년대에도 뉴스레터가 나왔고(주론 본문 참조) 제2호는 2025년 재창간
     기준이다. 「Since 1993 · 제2호」가 '창간 이래 두 번째'로 읽힐 여지가 있다.
     정확히 하려면 「재창간 제2호」. PI 확인 대기.
...
/* 첫 화면 배너 — **사진을 자르지 않는다.** (PI 2026-09-23)
   경위: 9/21에는 넓은 모니터에서 배너가 화면을 다 덮어 발간사가 보이지 않는
   문제를 '사진을 본문 폭(960px)까지 줄여서' 풀었고, 그러자 1366px 노트북에서
   사진이 섬처럼 놓여 표지 구실을 못 했다. 9/22에는 반대로 '가로를 화면 끝까지
   쓰고 세로를 잘라내는' 방식으로 바꿨다(3.2:1). 그런데 9/23 임원 회람에서
   사진을 찍으신 고전 교수가 「아래부분이 좀 나오도록 하면 풍광이 좋겠다」고
   하셨다 — 잘려 나가던 것이 앞쪽 나무울타리와 풀밭, 곧 근경이었다.
   덜 자르는 것으로는 끝이 나지 않는다(화면이 낮으면 max-height가 걸려 다시
   잘린다). 그래서 **자르지 않고, 대신 폭을 화면 높이에 맞춰 정한다**:
       폭 ≤ 화면높이 62% × 2.2216(원본 1173×528의 가로세로비)
   이러면 사진 전체가 늘 들어오고, 배너 높이는 화면의 62%를 넘지 않아 발간사가
   첫 화면에 걸린다. 값은 9/21에 물렸던 '본문 폭까지 줄이기'와 다르다 — 배너
   폭이 1058~1488px로 본문(960px)보다 항상 넓어 섬처럼 보이지 않는다.
   양옆 여백은 머리글과 같은 녹색이라 띠가 아니라 판면으로 읽힌다.
   양옆 띠 실측: 1920px 화면 216px · 1600px 180px · 1366px 154px · 900px 37px.
   모바일(640px 이하)은 폭을 가득 쓴다. */
@media (min-width: 641px) {
    .masthead-hero-img { background: var(--green-dark); }
    .masthead-hero-img img {
        max-width: calc(62vh * 2.2216);
        margin-inline: auto;
    }
}
.masthead-hero-img img {
...
.masthead-meta {
    ...
    gap: 0.5rem;
    flex-wrap: wrap;
    /* 발행처와 창간연도를 한 꼴로 — 자간·굵기·색이 갈리면 한 줄이 두 조각처럼
       보인다. 세 속성을 여기에 두어 이 줄에 무엇을 더 넣어도 같이 받게 한다.
       (PI 2026-09-22) */
    letter-spacing: 0.12em;
    font-weight: 600;
}
...
/* 세로로 긴 사진은 높이를 묶는다 (임원 회람 피드백 2026-09-23 — 양유정 총무:
   조천 탐방기 단체사진이 크게 느껴진다).
   원인은 그 사진만의 문제가 아니라 지면 규칙이 없었던 것이다. 본문 폭은 896px
   이고 폭에만 맞추면 3:4 사진은 세로 1,195px — 한 화면을 통째로 덮는다. 가로
   사진(2.16:1)은 414px이니 같은 지면에서 사진 크기가 세 배 가까이 갈렸다.
   높이를 560px로 묶으면 3:4 사진은 420×560이 되어 가로 사진과 무게가 맞는다.
   세로 사진은 이 호에 8장 있고(양유정·홍지환·황현철·김지원·전새미·석진아 등)
   전부 같은 규칙으로 줄어든다 — 한 장만 고치면 지면이 다시 어긋난다.
...
@media (max-width: 640px) {
    /* 좁은 화면에서는 탭을 가로로 흘리지 않고 줄바꿈한다.
       가로 스크롤은 표시가 없어 뒤쪽 탭(활동 소개)을 아무도 찾지 못한다. */
    .nav-inner {
    ...
    .nav-brand { font-size: 0.72rem; width: 100%; text-align: center; }
    .toc-list { flex-wrap: wrap; justify-content: center; row-gap: 0.15rem; }
    /* 일곱 칸을 한 줄에 넣는다 (PI 2026-09-23 — 「회비 납부」가 생겨 두 줄이 됐다).
       줄이는 것은 글자보다 좌우 여백이 먼저다 — 여백 9.6px는 한 줄에 7칸을
       넣을 때 67px을 잡아먹는다. 여백을 먼저 3~4px로 좁히고, 모자라는 만큼만
       글자를 줄인다(360px에서 11.3px, 390px에서 12.3px, 412px 이상 12.8px 그대로).
       320px 아래로는 한 줄을 포기하고 두 줄로 둔다 — 거기서 더 줄이면 글자가
       10px 밑으로 내려가 읽을 수 없다.
       flex-wrap:wrap은 그대로 둔다 — 예상 못 한 폭에서는 가로로 흘려 탭을
       감추지 말고 줄을 바꿔야 한다(뒤쪽 탭을 아무도 찾지 못한다).
       한 줄이 되어 남은 높이는 칸을 누르기 쉽게 쓴다: 36px → 40px. */
    .toc-list a {
...
<head>
  ...
  <style>{CSS}</style>
  <!-- 스크립트가 막힌 환경(메신저 내장 뷰어 등)에서도 내용이 보이게 한다.
       탭 전환 대신 모든 꼭지를 이어서 펼친다. 빈 화면보다는 낫다. -->
  <noscript><style>
    .view {{ display: block !important; }}
